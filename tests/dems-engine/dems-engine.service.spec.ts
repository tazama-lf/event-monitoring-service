...
describe('DemsEngineService', () => {
  ...
  describe('handleMessage', () => {
    ...
    it('should persist the raw message with a top-level TenantId while notifying with the Payload envelope (issue #83)', async () => {
      const testPayload = {
        name: 'John',
        age: 30,
        FIToFIPmtSts: {
      ...
      // Persistence stores the raw ISO message at the document root PLUS a top-level TenantId,
      // matching what raw_history's generated columns read (e.g. pacs008.tenantid is generated
      // from `document ->> 'TenantId'`). No `{ Payload: ... }` envelope.
    ...
    it('should cache the DataCache in Redis for a first-leg message (no related_transaction on this endpoint) with a usable EndToEndId (issue #82)', async () => {
